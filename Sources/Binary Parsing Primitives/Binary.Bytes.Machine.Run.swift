        ...
        interpreterLoop: while true {
            ...
            if let handle = pendingHandle {
                ...
                case .map(let transform):
                    pendingHandle = arena.allocate(transform.apply(value))

                case .tryMap(let transform):
                    do {
                        pendingHandle = arena.allocate(try transform.apply(value))
                    } catch {
                ...
                case .sequence(.combine(let firstHandle, let combine)):
                    pendingHandle = arena.allocate(combine.combine(arena.release(firstHandle), value))
                ...
                case .optional(_, let wrapSome, let noneHandle):
                    ...
                    pendingHandle = arena.allocate(wrapSome.apply(value))
            ...
            case .leaf(let instruction):
                ...
                case .take1:
                    ...
                    else { pendingHandle = arena.allocate(Value.make(input.removeFirst())) }
                ...
                case .take(let n):
                    if remaining < n { instructionError = .insufficientBytes(need: n, have: remaining) }
                        ...
                        for _ in 0..<n { bytes.append(input.removeFirst()) }
                        pendingHandle = arena.allocate(Value.make(bytes))
                    }

                case .skip(let n):
                    if remaining < n { instructionError = .insufficientBytes(need: n, have: remaining) }
                    else { input.removeFirst(n); pendingHandle = arena.allocate(Value.make(())) }
                ...
                case .u16le:
                    if remaining < 2 { instructionError = .insufficientBytes(need: 2, have: remaining) }
                        ...
                        let b0 = UInt16(input.removeFirst())
                        let b1 = UInt16(input.removeFirst())
                ...
                case .u32le:
                    if remaining < 4 { instructionError = .insufficientBytes(need: 4, have: remaining) }
                        ...
                        let b0 = UInt32(input.removeFirst())
                        let b1 = UInt32(input.removeFirst())
                        let b2 = UInt32(input.removeFirst())
                        let b3 = UInt32(input.removeFirst())
                ...
                case .u64le:
                    if remaining < 8 { instructionError = .insufficientBytes(need: 8, have: remaining) }
                        ...
                        var result: UInt64 = 0
                        for i in 0..<8 {
                            result |= UInt64(input.removeFirst()) << (i * 8)
                        }
                ...
                case .u64be:
                    if remaining < 8 { instructionError = .insufficientBytes(need: 8, have: remaining) }
                        ...
                        var result: UInt64 = 0
                        for _ in 0..<8 {
                            result = (result << 8) | UInt64(input.removeFirst())
                        }
                        pendingHandle = arena.allocate(Value.make(result))
                    }
                ...
                case .i8:
                    if remaining < 1 { instructionError = .insufficientBytes(need: 1, have: remaining) }
                    else { pendingHandle = arena.allocate(Value.make(Int8(bitPattern: input.removeFirst()))) }
